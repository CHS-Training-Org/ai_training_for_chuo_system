...
| **学習者（中堅）** | STEP-01・02 を必須で実施し、STEP-03 は両方のスキルに習熟していれば任意確認・スキップ可（詳細は [curriculum.md §学習パスマップ](./curriculum.md#path-map) 参照） |

> **用語の統一**：「オーナー」と「リポジトリ管理者」は同一アクターを指します（リポジトリ設定管理を担う「リポジトリオーナー」）。運用責任マトリクス（役割 × 責務）の詳細は [operations-guide.md §役割分担](./operations/operations-guide.md#roles) を参照してください。
