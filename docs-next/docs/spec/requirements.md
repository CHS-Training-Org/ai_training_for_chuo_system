...
ドメイン用語の定義は [用語集](../learn/glossary.md) に集約しました。
...
| [UC-09](#uc-09) | 管理者が予約実績を CSV で出力する | ADMIN | §帳票 |
...
### UC-09：管理者が予約実績を CSV で出力する {#uc-09}

#### 機能要件

| # | 要件 |
|---|------|
| RPT-01 | ADMIN は予約実績を CSV 形式でダウンロードできる |
| RPT-02 | CSV には予約 ID・リソース名・申請者名・開始日時・終了日時・目的・承認状態を含む |
| RPT-03 | 出力対象は期間（開始日時・終了日時、いずれも任意で片方のみの指定も可）と承認ステータス（任意・複数指定可）で絞り込める。期間の判定は予約の開始日時のみで行い、両方省略時は全期間、両端を指定した場合は両端を含む |
| RPT-04 | 出力範囲は常に全ユーザーの予約とする（ADMIN 限定機能のため行レベルの絞り込みは行わない） |
| RPT-05 | MEMBER・APPROVER はこの機能にアクセスできない（403 Forbidden） |
| RPT-06 | CSV は UTF-8 の BOM 付きで出力し、Excel で開いたときに日本語ヘッダと日時が正しく表示される |

> 出力件数に上限は設けない。大量データでもメモリを圧迫しないよう、サーバー側はストリーミングで出力する。
...
[ARCHITECTURE.md](../reference/architecture.md) の標準アーキテクチャ各レイヤーと BookFlow での実装の対応。
