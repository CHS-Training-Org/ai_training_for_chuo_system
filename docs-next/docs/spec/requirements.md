...
| RES-04 | リソース詳細（名称・カテゴリ・定員・場所・説明・承認フロー要否・設備一覧・利用上の注意）を閲覧できる。設備一覧・利用上の注意は未登録時は非表示でよい |
...
| # | 要件 |
|---|------|
| RES-05 | ADMIN はリソースを新規登録できる（名称・カテゴリ・定員・場所・説明・`requires_approval`・`is_active`・設備一覧・利用上の注意を入力） |
