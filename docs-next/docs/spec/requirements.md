...
| RES-09 | リソース一覧を名称・定員・登録日時のいずれかで並び替えられる（昇順・降順を選択可）。未指定時は登録日時の昇順を維持する |
| RES-10 | リソース詳細画面で、週表示（月曜始まり、0 時〜24 時の 1 時間刻み）と月表示（日単位の要約）を切り替えられるカレンダー形式の空き状況を確認できる |
| RES-11 | カレンダーの表示期間を前後に移動すると、その期間の占有予約を再取得して表示を更新する |
| RES-12 | カレンダーの空き枠をクリックすると予約申請フォームへ遷移し、開始日時（1 時間単位）が初期値として設定される。予約済み枠はクリックできない |
| RES-13 | 月表示の日セルをクリックすると、画面遷移せずその日を含む週の週表示に切り替わる |
