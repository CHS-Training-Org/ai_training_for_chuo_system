...
- 競合する課題：[多段階承認フロー](../multi-step-approval)。両課題とも `ApprovalService.createStep()`（`backend/.../application/ApprovalService.java`）を別方向に拡張するため、並行着手は非推奨。一方を完成させてからもう一方に着手する。
- 推奨着手順序：本課題の完成後に [既存機能の E2E テスト追加](../e2e-test-coverage) で承認ルーティングのリグレッションをカバーするとよい。
