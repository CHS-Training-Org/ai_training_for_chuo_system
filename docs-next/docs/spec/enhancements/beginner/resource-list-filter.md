...
- 推奨着手順序：本課題（キーワード検索）を**先に着手**するとよい。後続として [リソース一覧のソート順選択](./resource-list-sort.md)（本課題の完了を前提とする）・[OpenAPI クライアント自動生成](../advanced/openapi-client-gen.md)・[既存機能の E2E テスト追加](./e2e-test-coverage.md) がある。
