...
- 競合する課題：[リソース詳細画面の情報拡充](../beginner/resource-detail-info.md)・[リソース画像アップロード](../advanced/resource-image-upload.md)。いずれもリソース詳細画面（`/resources/{id}`）を共有して変更するため、同時並行ではマージ競合の可能性がある。
