...
経験年数やレベルによる学習パスの分岐は設けていません。全員が同じ順序で同じ課題に取り組みます（[curriculum.md §学習パスマップ](../learn/curriculum.md#path-map)）。
...
- [ARCHITECTURE.md](../reference/architecture.md) に記載されたアーキテクチャの全レイヤー（BFF・バックエンド・DB・AWS サービス）を体験できる
...
| **学習者** | STEP-01〜05 の順で必須課題に取り組む（詳細は [curriculum.md §学習パスマップ](../learn/curriculum.md#path-map) 参照） |
...
プロジェクト・技術用語の説明は [用語集](../learn/glossary.md) に集約しました。
