...
更新責任：仕様変更を伴う実装を行う本人（学習者を含む）。セルフレビュー責任も同様に本人が負う。詳細は[仕様更新ルール](#spec-first)を参照
...
| [enhancements/](./enhancements/index.md) | エンハンス課題のビジネス要求シート集。配置規約・原則を定義 | 学習者・運営者 |
| [aidlc-adoption.md](../reference/aidlc/adoption.md) | AI-DLC 採用台帳。AWS Labs AI-DLC から取り込んだ要素・反映先・状態・上流同期手順を管理（BookFlow 機能仕様ではなく上流参照の管理台帳） | 運営者 |
...
`docs-next/docs/spec/` を「真実の源」とします。機能の追加、変更、削除を行うときは、**実装より先に仕様を更新する**（Spec-first）ことを原則とします。

実装と仕様の乖離を防ぎ、仕様差分を見れば「何を作ろうとしているか」が先にわかる状態を作るためです。
...
3. 仕様更新をコミットに含める。実装コミットに同梱してもよいし、独立した `docs(spec): <変更内容>` コミットに分けてもよい（分ける場合は PR の先頭コミットにする）
4. 実装・テストを行う
...
- Claude Code の使い方: [ai-tools-guide.md](../learn/ai-tools-guide.md)
