...
| キーワード検索欄 | リソース名・説明文への部分一致検索（大文字小文字を区別しない）。「絞り込む」送信時に `GET /api/resources` の `keyword` パラメータとして送出し、カテゴリ・期間フィルタと AND 条件で組み合わせる。空欄で送信するとキーワード条件は解除される |
...
| 利用目的 | テキスト入力欄（必須） |
| 参加人数 | 数値入力欄（任意） |
| 「申請する」ボタン | フォーム送信（`POST /api/reservations`、`draft` を指定しない） |
| 「下書き保存」ボタン | フォーム送信（`POST /api/reservations`、`draft: true`）。入力必須項目は「申請する」と同じ |
...
| ルール | 条件 |
|--------|------|
| リソース必須 | リソース未選択は申請不可（下書き保存も同様） |
| 開始・終了日時必須 | 両方の入力が必要（下書き保存も同様） |
| 終了 > 開始 | 終了日時は開始日時より後であること |
| 重複予約 | 「申請する」で 409 Conflict が返された場合、「指定した時間帯は既に予約が入っています」を表示。「下書き保存」では重複チェックを行わないため発生しない |
...
- 下書き保存成功 → マイ予約一覧（`/reservations`）へリダイレクト。ステータスは `DRAFT`
...
| 要素 | 説明 |
|------|------|
| ステータスフィルター | 「すべて」／「下書き（DRAFT）」／「承認待ち（PENDING）」／「承認済み（APPROVED）」／「却下（REJECTED）」／「キャンセル済み（CANCELLED）」のタブ |
| 予約一覧テーブル | リソース名・日時・目的・ステータスバッジを表示 |
| ステータスバッジ | `DRAFT`=グレー・`PENDING`=黄・`APPROVED`=緑・`REJECTED`=赤・`CANCELLED`=グレー |
...
| 編集ボタン | `PENDING` または `DRAFT` の予約かつ申請者本人の場合に表示。予約編集画面（`/reservations/{id}/edit`）へ遷移 |
| キャンセルボタン | `PENDING`／`APPROVED`／`DRAFT` の予約に対して表示。クリック後に確認ダイアログを表示 |
| 「正式申請」ボタン | `DRAFT` の予約かつ申請者本人の場合に表示。クリックすると保存済みの内容のまま `PUT /api/reservations/{id}`（`submit: true`）を呼び出し、`requires_approval` に応じて `APPROVED` または `PENDING` へ遷移する。確認ダイアログを表示する |
...
|--------|-------------|
| MEMBER | 自分の予約のみ |
| APPROVER | すべての予約（`DRAFT` は自分のもののみ） |
| ADMIN | すべての予約 |

他人の予約に MEMBER がアクセスした場合は 403 エラーを表示する。他人の `DRAFT` に APPROVER がアクセスした場合も同様に 403 エラーを表示する（`PENDING` 以降の予約は既存どおり APPROVER は全件閲覧可）。キャンセル操作は申請者本人または ADMIN のみ可能。編集操作・正式申請は申請者本人（MEMBER / APPROVER）のみ可能。
...
| 「更新する」ボタン | フォーム送信（`PUT /api/reservations/{id}`、`submit` は指定しない）。`DRAFT` の予約は更新後も `DRAFT` のまま（正式申請は予約詳細画面の「正式申請」ボタンから行う） |
| 「戻る」リンク | 予約詳細（`/reservations/{id}`）へ戻る |

> 対象予約が `DRAFT` の場合、見出しを「下書きを編集する」とし、「下書きの内容を変更して保存し直せます（正式申請は予約詳細ページから行います）」という説明を表示する。
...
| 条件 | 動作 |
|------|------|
| `PENDING`／`DRAFT` 以外の予約 | 422 エラー（API が返す） |
