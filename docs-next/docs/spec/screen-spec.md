...
| 2 | `/resources` | リソース一覧 | カテゴリ別一覧・キーワード検索・空き確認日時入力 | 全ロール（要認証） |
...
| キーワード検索欄 | 名称・説明文への部分一致（大文字小文字を区別しない）で絞り込むテキスト入力欄（`GET /api/resources` の `keyword` フィルタ仕様に準拠）。カテゴリ・空き確認フィルタとは AND 条件で組み合わさる。空にして絞り込むとキーワード条件は解除される |
