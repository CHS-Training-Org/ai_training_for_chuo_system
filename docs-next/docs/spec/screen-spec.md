...
| キーワード検索欄 | リソース名・説明文への部分一致検索（大文字小文字を区別しない）の入力欄。「絞り込む」送信時に `keyword` を URL パラメータとして付与する（`GET /api/resources` の `keyword` フィルタ仕様に準拠）。空にして送信するとキーワード条件が解除される。カテゴリ・空き確認フォームと AND 条件で組み合わせられる |
