...
sidebar_position: 4
title: 画面仕様書
description: BookFlow の全 12 画面のレイアウト・操作フロー・バリデーションを定義した画面仕様書
tags:
  - spec
  - screen
  - ui
...
| 12 | `/admin/reports` | 帳票出力 | 期間・ステータスを指定して予約実績を CSV ダウンロード | ADMIN のみ |
...
| 要素 | 説明 |
|------|------|
| 開始日入力 | `type="date"`。任意入力 |
| 終了日入力 | `type="date"`。任意入力 |
| 承認ステータス選択 | セレクト（すべて／`DRAFT`／`PENDING`／`APPROVED`／`REJECTED`／`CANCELLED`）。デフォルトは「すべて」 |
| 「CSV ダウンロード」ボタン | `GET /api/reports/reservations/csv` を呼び出し、レスポンスをファイルとして保存する。ダウンロード中はボタンを無効化する |
| 「リセット」ボタン | 入力値を初期状態に戻す |

#### バリデーション

| ルール | 条件 |
|--------|------|
| 開始日・終了日の整合性 | 開始日が終了日より後の場合、API からの `400 VALIDATION_ERROR` をエラーメッセージとして表示する（フロントエンドでの事前検証は行わない） |

#### ダウンロード後の挙動

- 成功時：ブラウザにファイルが保存され、成功メッセージを表示する。対象が0件でもヘッダ行のみの CSV として正常にダウンロードされる（エラー扱いにしない）
- 失敗時：エラーの種類に応じたメッセージを表示する（権限エラー・セッション切れ・その他のエラーを区別する）
