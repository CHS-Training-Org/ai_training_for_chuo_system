...
| 2 | `/resources` | リソース一覧 | キーワード検索・カテゴリ別一覧・空き確認日時入力 | 全ロール（要認証） |
...
| 要素 | 説明 |
|------|------|
| キーワード入力欄 | リソース名・説明文への部分一致検索（`GET /api/resources` の `keyword` パラメータ）。プレースホルダーは「リソース名・説明で検索」、入力上限 100 文字。空のまま「絞り込む」を押すとキーワード条件のみ解除される |
...
| ページネーション | page/size 方式（デフォルト 20 件/ページ）。キーワード・カテゴリ・空き確認期間の絞り込み条件はページ送り後も維持される |

#### バリデーション

| ルール | 条件 |
|--------|------|
| キーワードの長さ | 前後の空白を除いて 100 文字以内。超過時は `400 Bad Request`（`code: VALIDATION_ERROR`） |
| 空き確認期間の指定 | 開始日時・終了日時は同時に指定する。片方のみの場合は `400 Bad Request`（`code: VALIDATION_ERROR`） |

> 絞り込み条件は URL のクエリパラメータとして保持する。条件を指定した状態の URL を開くと、各入力欄に値が復元される。
