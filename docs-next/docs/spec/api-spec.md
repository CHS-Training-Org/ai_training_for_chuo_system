...
| メソッド | パス | 概要 | 権限 |
|--------|------|------|------|
| GET | `/api/resources` | リソース一覧（カテゴリ・キーワード・空き日時でフィルタ可） | 全ロール |
...
### `GET /api/users/me`（自プロフィール取得） {#get-api-users-me}
#### リクエスト
...
```http
GET /api/resources?category=ROOM&keyword=projector&from=2025-06-01T09:00:00&to=2025-06-01T18:00:00&page=0&size=20
Authorization: Bearer <JWT>
```
...
| `keyword` | string | ❌ | `name` または `description` への部分一致（大文字小文字を区別しない）でフィルタ |
...
>
> `keyword` を指定した場合、`name` または `description` に部分一致（大文字小文字を区別しない）するリソースのみを返す。`category` / `from` / `to` とは AND 条件で組み合わさる。未指定時は全件が対象になる。
...
UserResponse 型定義は [`GET /api/users/me`](#get-api-users-me) を参照。
