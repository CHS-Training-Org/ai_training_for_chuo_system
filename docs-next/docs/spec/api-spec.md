...
| `keyword` | string | ❌ | リソース名・説明文への部分一致検索（大文字小文字を区別しない） |
...
> `from` / `to` を指定した場合、当該時間帯に `status IN ('PENDING', 'APPROVED')` の予約が存在しないリソースのみを返す（占有中のリソースは結果から除外される）。片方のみ指定した場合は `400 Bad Request`（`code: VALIDATION_ERROR`）。ADMIN は `is_active = false` のリソースも含む。`keyword` を指定した場合、`resources.name` または `resources.description` のいずれかに部分一致するリソースのみを返す。`category`・`from`/`to`・`keyword` は AND 条件で組み合わされる。
