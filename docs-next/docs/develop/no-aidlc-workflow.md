...
**起動の合図**：チャットで `/aidlc` と入力した場合、または「AI-DLCで進めて」のようにAI-DLCを名指しで使う意図を伝えた場合にだけ自動進行が始まります。課題の相談や実装の依頼など、それ以外の通常の会話では基本的に起動しません。

**起動した瞬間に気づく方法**：エンジンが起動すると、「👋 Welcome to AI-DLC」という挨拶メッセージが応答の冒頭に表示されます。これが出た場合はエンジンが起動しています。

**後から確認する方法**：気づかなかった場合は、次の点を確認してください。

- `Docs/spec/aidlc-audit.md`（AI-DLCの実行記録。起動した直後に追記される）に新しいエントリが増えていないか
- `git status` の Untracked files に、`Docs/spec/aidlc-docs/`（AI-DLCが作る設計成果物の置き場）配下の新規ファイルが出ていないか
- コミットまで進んでしまっている場合に備え、自分のトランクブランチとの差分（`git diff learner/<GitHubユーザー名>/main...HEAD --stat`）にも `Docs/spec/aidlc-state.md`（進捗トラッカー）や上記のファイルが含まれていないか

いずれも変更がなければ、エンジンは起動していません（記録前に異常終了した場合等の例外はありますが、通常はこれで判別できます）。
