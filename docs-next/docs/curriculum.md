...
STEP-01（環境構築）・STEP-02（リポジトリ運用・開発フロー理解）は、本リポジトリ固有の環境とプロセスであるため、若手・中堅を問わず全員が必須で実施します。  
...
| パス | 進め方 |
| ---- | ------ |
| 若手向け | STEP-01 → 02 → 03 を**順番に必須**で実施 → Beginner レベルの選択課題へ |
...
難易度別の選択課題カタログ（Beginner / Intermediate / Advanced）は **[選択課題カタログ](./develop/enhancement-catalog.md)** を参照してください。

- **若手**は STEP-03 完了後に [Beginner の課題](./develop/enhancement-catalog.md#beginner) から始めることを推奨します。
- **中堅**は [Intermediate / Advanced の課題](./develop/enhancement-catalog.md#intermediate) から選んでください。
- 選択課題に着手する際は、[コードベース理解ガイド](#codebase-understanding)を必要な範囲で参照してください。

---

## 必須ステップ課題
若手が最初に取り組む 3 つの課題です。**順序性があり**、前の STEP の完了を前提に次へ進みます。  
...
選択課題の進め方は [dev-workflow.md §標準開発フロー](./develop/dev-workflow.md#flow) を参照してください。選択課題は PR に `@claude pr-review` とコメントして AI レビューを受け、総合判定が「完了」になることがタスク完了の条件です（[review-criteria.md §AI レビューとの対応](./develop/review-criteria.md#ai-review)）。
...
手順の詳細は [getting-started.md](./getting-started.md)を参照してください。トラブル時は [troubleshooting.md](./develop/troubleshooting.md) も確認してください。
...
| 推定工数 | 半日 |
| AI 活用例 | Claude Code に [dev-workflow.md](./develop/dev-workflow.md) を読み込ませ、標準フローを自分の言葉で要約させて理解を確認する |
| 完了条件 | 下記の確認項目をすべて満たし、自分の言葉で説明できる（最初に着手する選択課題の PR に記載する） |

実装に着手する前に、リポジトリの運用ルールを把握します。次の 5 点を確認してください。

- **標準開発フロー**：[dev-workflow.md §標準開発フロー](./develop/dev-workflow.md#flow) を読み、「ビジネス要求シート（Issue）選択 → ブランチ作成 → `/aidlc` 起動・計画提示・チャットでセルフ承認 → Spec-first で仕様更新 → 縦切り実装 → セルフレビュー → PR → `@claude pr-review` で AI レビュー → セルフレビュー・マージ」という**自己完結の流れ**を説明できる。
- **AI レビューの位置づけ**：[review-criteria.md §AI レビューとの対応](./develop/review-criteria.md#ai-review) を読み、AI レビューの総合判定が「完了」になることがタスク完了の条件であること、観点3（理解度チェック）は4択への回答が必要で少なくとも2回のやり取りを要することを理解する。
- **AI-DLC の考え方**：[dev-workflow.md §AI-DLC と BookFlow フローの対応](./develop/dev-workflow.md#aidlc-mapping) を読み、plan-first（`/aidlc` を起動して AI-DLC エンジンに計画を立てさせ、提示された計画に納得してから実装する）の狙いを理解する。
- **ブランチの切り方**：[coding-conventions.md §共通方針](./develop/coding-conventions.md#common) に従い、`feature/<GitHubユーザー名>/<issue番号>-<short-desc>` 形式でブランチを作成できる。issue 番号は要件シートには記載されておらず、Issue を起票した時点で GitHub が採番するため、起票後に Issue の URL やタイトル横の表示で確認する。
...
アーキテクチャ全体像は [ARCHITECTURE.md](./architecture.md)、各レイヤーの責務とテスト規約は [coding-conventions.md](./develop/coding-conventions.md) を参照してください。[標準開発フロー](./develop/dev-workflow.md#flow) でブランチを作成したあと、実装に入る前に確認しておくと、その後の Workflow Planning での計画が立てやすくなります。
