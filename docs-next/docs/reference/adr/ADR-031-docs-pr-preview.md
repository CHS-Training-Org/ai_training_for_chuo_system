...
- **見出し単位の抽出**（2026-09-04 追記）：変更ページの通知に「変更された見出し」の列を設け、**見出しアンカー付きの直リンク**を列挙する。ページ単位のリンクでは、長いページのどこが変わったかをレビュアーが探す必要が残るため。

  - 変更行の位置は PR files API のレスポンスに含まれる `patch`（unified diff）のハンクヘッダーから head 側の行番号として取り、head 側の本文を走査して直前の見出しへ割り当てる。追加の API 呼び出しは不要。
  - 見出しアンカーは、明示 ID（`## 見出し {#id}`）があればそれを使い、無ければ `github-slugger` で算出する。Docusaurus 自身が同じライブラリで ID を生成するため結果は一致する（`docs-next/docs/` 配下の全 703 見出しでビルド済み HTML の ID と一致することを確認した）。算出した ID が `pages-root/` のビルド済み HTML に存在しない場合はリンクにせずラベルだけを表示し、リンク切れは出さない。
  - コードフェンスの内側と frontmatter は走査から除外する。除外しないと、シェルのコメント行（`# ...`）を見出しとして拾う。
  - 見出しに割り当てられない変更（frontmatter のみの更新等）、新規追加・移動されたページ、素の HTML である運営ノートは、見出しを列挙せずページ単位の表示にとどめる。
  - 1 ページあたり 5 件、コメント全体で 60 件を上限とする（超過分は「ほか N 件」）。GitHub のコメント上限（65,536 文字）に収めるため。
