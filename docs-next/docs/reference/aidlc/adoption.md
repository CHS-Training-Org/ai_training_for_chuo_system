...
- **vendored スナップショット**: [`vendor/aidlc-rules/`](https://github.com/CHS-Training-Org/ai_training_for_chuo_system/tree/main/vendor/aidlc-rules)（逐語コピー・出典は `PROVENANCE.md`）
- **BookFlow 翻案済みステージファイル**: [`.aidlc-rule-details/`](https://github.com/CHS-Training-Org/ai_training_for_chuo_system/tree/main/.aidlc-rule-details)（パス翻案済み活性資産）
...
2. [`vendor/aidlc-rules/`](https://github.com/CHS-Training-Org/ai_training_for_chuo_system/tree/main/vendor/aidlc-rules) の各ファイルを新しい上流コミットの内容と diff する（vendored スナップショットが diff の基準）。
...
- BookFlow の標準フロー・フェーズ詳細: [開発ワークフローガイド](../../develop/dev-workflow.md)
