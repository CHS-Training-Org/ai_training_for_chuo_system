...
| `難易度：初級` | 難易度 Beginner | [カタログ Beginner 節](../develop/enhancement-catalog.md#beginner) の課題 | メンター |
| `難易度：中級` | 難易度 Intermediate | [カタログ Intermediate 節](../develop/enhancement-catalog.md#intermediate) の課題 | メンター |
| `難易度：上級` | 難易度 Advanced | [カタログ Advanced 節](../develop/enhancement-catalog.md#advanced) の課題 | メンター |
...
## ラベルマッピング規則
起票時に付与するラベルの組み合わせを以下の通りに定めます。必須ステップ（STEP-01〜03）は GitHub Issue を起票しないため、対象は選択課題のみです。
...
| `難易度：初級` / `難易度：中級` / `難易度：上級` | [カタログの難易度節](../develop/enhancement-catalog.md#catalog) から決定 | メンター |
...
1. GitHub の「Issues → New issue」で「選択課題（エンハンス）」テンプレートを選ぶ（必須ステップ STEP-01〜03 は Issue を起票しない）
