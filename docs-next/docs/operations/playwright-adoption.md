...
最初の導入は、必須課題とは切り離した独立したチュートリアルとする。既にある予約申請画面（`/reservations/new`）を題材に、試験観点を作るところからテストを通すところまでを一続きで扱う。
...
太い枠が、人が判断を担う工程である。他の工程はエージェントに任せ、学習者は出てきたものを仕様に照らして確かめる側に回る。

この形を選んだのは、必須課題の流れに組み込むよりも用意する機構が少なく、9月末までに運用に載せられるからである。必須課題への組み込みは、独立したチュートリアルが回るようになってから検討する。組み込む場合は、3 つの層のうち試験観点だけを実装より前に置く形になる。課題ごとのビジネス要求シートには受入条件が書かれているため、これを根拠に試験観点を作り、エージェントの成果物はその結合テストが通るかどうかで判定される。実装より先に仕様を更新するという既存の Spec-first 運用に、実行可能性を与える形でもある。

なお、結合テストを追加する課題は選択課題カタログの初級に[既存機能の E2E テスト追加](../spec/enhancements/beginner/e2e-test-coverage.md)として存在する。この課題との関係は未決である。
...
9月末までに実施可能なチュートリアルを載せることを優先し、工数が最も小さい選択を取るという前提で、次を決めた。

| # | 決めたこと | 内容 |
|---|---|---|
| 1 | カリキュラム上の扱い | 必須課題とは独立したチュートリアルとする |
| 2 | 対象 | 予約申請画面（`/reservations/new`）の結合テスト |
| 3 | 範囲 | フロントエンドに限る |
| 4 | 到達目標 | 学習者が、AI を使って結合テストの試験観点とテストコードを作り、実行して通せる状態になる |

この 4 つから次が従う。検証の対象は学習者が書いた実装ではなく既存のベース実装であり、試験観点は完成したものを題材に作る。したがってこのチュートリアルで身につくのは、エージェントの成果物を検証する手順そのものである。

---

## 未決事項 {#open-questions}

決まったことを踏まえて残る論点を、決める順に並べる。
...
| 4 | 試験観点を作る手段 | 既にある SKILL を使うのか、Playwright 公式のテストエージェントを使うのか、両方を併記するのか |

1 は、ローカル環境ではブラウザからのサインインが成立しない（cognito-local が http で動き、認証クライアントのリダイレクトが失敗する）ことによる。2 は、現在の実装に `data-testid` が 1 つも入っていないことによる。この 4 つは机上では決まらないため、実際に動かして確かめる。
...
| 6 | 試験観点のレビュー基準 | 生成された試験観点が妥当かを、誰がどの基準で判断するのか |
| 7 | CI での実行 | 独立したチュートリアルの段階では CI に載せるのか、ローカル実行に留めるのか |
| 8 | 既存の選択課題の扱い | 内容を書き換えて流用するのか、そのまま残すのか |

5 は、方針としては完了条件に加えたい。ただし、学習者が書いたテストが安定して動く状態まで到達できるかが分かっていない。動かないものを完了条件にはできないため、A の検証結果を見てから判断する。

6 は、SKILL であれテストエージェントであれ、成果物を生成する手順を固定するだけで、生成されたものが正しいかどうかは保証しないことによる。試験観点が仕様を網羅しているか、期待結果が妥当かは、別の仕組みで確かめることになる。
...
- 未決事項 A を実際に動かして確かめる。検証は本ページとは別のブランチで行う
- 検証結果を踏まえてチュートリアル本体を作る
- 現在チュートリアルを実施中の学習者に試してもらい、最終修正を行う
- 決定内容を ADR として記録する（[ADR-009](../reference/adr/ADR-009-frontend-test-strategy.md) への追記か、新規 ADR かを含めて決める）
