...
# cognito-local は既定では起動しない（ロール別ログイン・保護エンドポイントの JWT 検証を使う場合のみ必要）
...
動作確認はサインイン画面の「開発専用ロール別ログインボタン」（`NODE_ENV !== 'production'` 時のみ表示、`src/server/actions/dev-auth.ts`）を使うこと。このボタンは内部で cognito-local に直接 InitiateAuth を投げるため、上記の cognito-local 起動が前提になる。
