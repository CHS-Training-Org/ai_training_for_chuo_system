 ...
 *
 * <p>
 * テストデータは {@link JdbcTemplate} で直接挿入・削除し、H2 インメモリ DB を使用する。 ロール別アクセス制御・リソース
 * CRUD・空き照会のレスポンス形状を確認する。
 *
 * <p>
 * テスト命名規約（ADR-018）: {@code methodName_condition_expectedBehavior}
 */
class ResourceControllerTest extends BaseControllerTest {
        ...
        private static final UUID ACTIVE_RESOURCE_ID = UUID.fromString("10000000-0000-0000-0000-000000000010");
        private static final UUID INACTIVE_RESOURCE_ID = UUID.fromString("10000000-0000-0000-0000-000000000011");
        private static final UUID RESERVATION_ID = UUID.fromString("10000000-0000-0000-0000-000000000020");
        private static final UUID PROJECTOR_RESOURCE_ID = UUID.fromString("10000000-0000-0000-0000-000000000013");
        ...
        void list_memberWithoutFilter_returnsActiveResourcesOnly() throws Exception {
                mockMvc
                                ...
                                .andExpect(jsonPath("$.content[?(@.id == '" + INACTIVE_RESOURCE_ID + "')]")
                                                .doesNotExist());
        }
        ...
        void list_withTimeRangeOverlappingReservation_excludesOccupiedResource() throws Exception {
                ...
                mockMvc
                                ...
                                .andExpect(status().isOk())
                                .andExpect(jsonPath("$.content[?(@.id == '" + ACTIVE_RESOURCE_ID + "')]")
                                                .doesNotExist());
        }
        ...
        void list_CategoryAndKeyword_returnResource() throws Exception {
                mockMvc
                                ...
                                .andExpect(status().isOk())
                                .andExpect(jsonPath("$.content[?(@.id == '" + ACTIVE_RESOURCE_ID + "')]").exists())
                                .andExpect(jsonPath("$.content[?(@.id == '" + PROJECTOR_RESOURCE_ID + "')]")
                                                .doesNotExist());
        }
