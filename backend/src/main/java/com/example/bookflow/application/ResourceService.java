...
public class ResourceService {
   ...
   * @param keyword  キーワード（nullの場合はフィルタしない）
  ...
  private Page<ResourceResponse> listPaginated(
    ...
    if (keyword != null && !keyword.isBlank()) {
      page = resourceRepository.findByNameContainingIgnoreCaseOrDescriptionContainingIgnoreCase(keyword, keyword,
          pageable);
    } else if (isAdmin) {
      page = category != null
  ...
  private Page<ResourceResponse> listWithAvailabilityFilter(
    ...
    // 3. 候補のうち占有済み予約があるリソース ID を特定（2 クエリ）
