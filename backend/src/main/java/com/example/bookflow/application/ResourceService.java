 ...
 *
 * <p>
 * カテゴリ 4 で導入する初の Service 層。業務ルールを集約し、Controller は薄く保つ。
 *
 * <ul>
 ...
 *
 * <p>
 * 時間帯の重複判定ロジック（{@link #overlaps}）はカテゴリ 5 の {@code 409 RESERVATION_CONFLICT}
 * チェックでも再利用する。
 */
@Service
@Transactional
public class ResourceService {
  ...
  private static final List<ReservationStatus> OCCUPIED_STATUSES = List.of(ReservationStatus.PENDING,
  ...
  private Page<ResourceResponse> listPaginated(
    ...
    if (isAdmin) {
      if (category != null || keyword != null) {
        page = resourceRepository.findByCategoryAndKeyword(category, keyword, pageable);
      } else {
        page = resourceRepository.findAll(pageable);
      }
    } else {
      if (category != null || keyword != null) {
        page = resourceRepository.findByCategoryAndKeywordAndIsActiveTrue(category, keyword, pageable);
      } else {
        page = resourceRepository.findByIsActiveTrue(pageable);
      }
    }
   ...
   *
   * <p>
   * 重複するリソース ID を一括取得（1 クエリ）し、候補リストから除外する。
   */
  private Page<ResourceResponse> listWithAvailabilityFilter(
    ...
    if (!candidateIds.isEmpty()) {
      Set<UUID> occupiedIds = reservationRepository
    ...
    List<ResourceResponse> content = start >= total
        ? List.of()
        : candidates.subList(start, end).stream().map(ResourceResponse::from).toList();
    return new PageImpl<>(content, pageable, total);
  }

  private List<Resource> fetchAllCandidates(ResourceCategory category, String keyword, boolean isAdmin) {
    if (isAdmin) {
      return (category != null || keyword != null)
          ? resourceRepository.findByCategoryAndKeyword(category, keyword)
          : resourceRepository.findAll();
    } else {
      return (category != null || keyword != null)
          ? resourceRepository.findByCategoryAndKeywordAndIsActiveTrue(category, keyword)
   ...
   *
   * <p>
   * {@code status IN ('PENDING', 'APPROVED')} の予約のうち、指定期間と重複する予約を返す。
   * 空きスロットの計算はフロントエンド側の責務。
   *
   * @param id   リソース ID
   ...
   *
   * <p>
   * 隣接（{@code existingEnd == from} または {@code to == existingStart}）は重複とみなさない。
   * 予約時間帯の重複判定（カテゴリ
   * 4・5 共通ロジック）。
   *
