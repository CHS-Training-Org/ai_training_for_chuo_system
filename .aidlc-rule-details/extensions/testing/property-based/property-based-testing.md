...
4. The finding MUST be logged in `Docs/spec/aidlc-audit.md` with the PBT rule ID, description, and stage context
...
If the user selected **Partial** enforcement during opt-in, only rules PBT-02, PBT-03, PBT-07, PBT-08, and PBT-09 are enforced. All other rules are treated as advisory (non-blocking). Log the enforcement mode in `Docs/spec/aidlc-state.md` under `## Extension Configuration`.
