 ...
 * - PENDING/DRAFT 以外の予約は編集対象外（notFound で弾く、BE も 422 を返す）。
...
export default async function ReservationEditPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  // ADMIN は PUT 権限なし（権限マトリクス）、他人の予約・PENDING/DRAFT 以外は対象外
  if (isAdmin || !isOwner || !["PENDING", "DRAFT"].includes(reservation.status)) {
    notFound();
  }
    ...
    <div className="space-y-6 max-w-2xl">
      ...
      <div>
        <h1 className="text-2xl font-bold">
          {isDraft ? "下書きを編集する" : "予約内容を編集する"}
        </h1>
        <p className="text-sm text-muted-foreground mt-1">
          {isDraft
            ? "下書きの内容を変更して保存し直せます（正式申請は予約詳細ページから行います）。"
