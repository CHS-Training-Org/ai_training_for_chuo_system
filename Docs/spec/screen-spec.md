...
| 10 | `/auth/signin` | サインイン | メールアドレス・パスワードでサインイン | 未認証のみ |
...
未認証ユーザー専用のサインイン画面。認証済みユーザーがアクセスした場合はダッシュボード（`/`）へリダイレクトする。

#### UI 要素

| 要素 | 説明 |
|------|------|
| メールアドレス入力欄 | `type="email"`・必須 |
| パスワード入力欄 | `type="password"`・必須 |
| サインインボタン | フォーム送信 |
...
| パスワード必須 | 「パスワードを入力してください」 |
| 認証失敗（Cognito エラー） | 「メールアドレスまたはパスワードが正しくありません」 |

#### 画面フロー

1. フォーム送信 → Better Auth 経由で Cognito に認証リクエスト
2. 認証成功 → ダッシュボード（`/`）へリダイレクト
3. 認証失敗 → エラーメッセージをフォーム上部に表示（フォームはリセットしない）
