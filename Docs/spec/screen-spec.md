...
| 2 | `/resources` | リソース一覧 | カテゴリ別一覧・並び替え・空き確認日時入力 | 全ロール（要認証） |
...
| 並び替えセレクト | 「登録日時が古い順」（デフォルト）／「登録日時が新しい順」／「名称順（昇順・降順）」／「定員順（昇順・降順）」の6択。選択値を `GET /api/resources` の `sort` パラメータとして付与する |
...
| 空き状況カレンダー | 週表示（月曜始まり、0 時〜24 時の 1 時間刻みグリッド）と月表示（日単位の要約セル、予約件数を表示）を切り替えられる。「前週/前月」「次週/次月」ボタンで表示期間を移動するたびに、その期間の `from`/`to` で `GET /api/resources/{id}/availability` を呼び直す。`status IN ('PENDING', 'APPROVED')` の予約と重複する枠はグレーアウトし、色だけでなくテキスト（「予約済」）でも判別できるようにする |
| カレンダーの空き枠クリック（週表示） | グレーアウトされていない 1 時間セルをクリックすると、予約申請フォーム（`/reservations/new?resourceId={id}&startAt={開始日時}`）へ遷移する。予約済み枠はクリックできない |
| カレンダーの日セルクリック（月表示） | 日セルをクリックすると、画面遷移せずその日を含む週の週表示に切り替わる |
| 空き状況リスト | 当日〜7 日後の占有時間帯（`status IN ('PENDING', 'APPROVED')` の予約。`GET /api/resources/{id}/availability` で取得）をリスト表示。占有なしの場合は「予約はありません」メッセージ。カレンダーの表示期間とは独立しており、常に当日〜7 日後を表示する |
...
> `/resources/{id}` からの遷移時は `resourceId` をクエリパラメータ（`?resourceId={id}`）で受け取り、リソース選択欄に初期値として設定する。カレンダーの空き枠クリックからの遷移時はあわせて `startAt`（`YYYY-MM-DDTHH:mm` 形式、1 時間単位）を受け取り、開始日時欄に初期値として設定する。
