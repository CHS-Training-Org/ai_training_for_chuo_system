...
| 要素 | 説明 |
|------|------|
| キーワード検索欄 | リソース名・説明文への部分一致検索（大文字小文字を区別しない）。「絞り込む」送信時に `keyword` を URL パラメータへ付与する（`GET /api/resources` の `keyword` フィルタ仕様に準拠）。空にして「絞り込む」を押すとキーワード条件を解除する |
...
| ページネーション | page/size 方式（デフォルト 20 件/ページ）。キーワード・カテゴリ・期間フィルタとAND条件で組み合わせられる |
