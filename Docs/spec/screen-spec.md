...
| 2 | `/resources` | リソース一覧 | カテゴリ別一覧・キーワード検索・並び替え・空き確認日時入力 | 全ロール（要認証） |
...
| キーワード検索欄 | リソース名・説明文を対象とするテキスト入力。入力値を `GET /api/resources` の `keyword` パラメータとして付与し、名称・説明文への部分一致（大文字小文字を区別しない）で絞り込む。カテゴリ・並び替え・空き確認の各フィルタとは AND 条件で組み合わさる。空欄で「絞り込む」を押すとキーワード条件は解除される |
