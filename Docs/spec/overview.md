...
audience: 学習者（若手・中堅以上）・メンター・リポジトリ管理者
references:
...
| ペルソナ | 前提スキル | 到達目標 |
|----------|-----------|---------|
| 若手エンジニア | プログラミング基礎あり、フルスタック未経験 | AI 補助のもとで既存コードを読み解き、小規模な機能改修ができる |
| 中堅エンジニア | 一定の開発経験あり、AI 駆動開発未経験 | AI ツールを使い分けながら独立した機能追加・設計ができる |
| メンター | 上記の指導経験 | 学習者の進捗管理・課題設計・任意のコードフィードバックができる |
...
| **メンター** | 課題設計・学習者サポート・任意のコードフィードバック（マージのブロッキング承認は行わない） |
| **AI エージェント** | ソフトウェア開発要求に対して INCEPTION（計画）→ CONSTRUCTION（実装）→ OPERATIONS（CI）を駆動する。学習者自身がレビュー・承認 |
| **学習者（若手）** | 必須ステップ課題 → 選択課題の順で実施 |
| **学習者（中堅）** | STEP-01・02 を必須で実施し、STEP-03 は両方のスキルに習熟していれば任意確認・スキップ可（詳細は [curriculum.md §学習パスマップ](../guide/curriculum.md#path-map) 参照） |
