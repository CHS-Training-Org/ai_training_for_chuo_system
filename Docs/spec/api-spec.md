...
```http
GET /api/resources?category=ROOM&keyword=会議室&from=2025-06-01T09:00:00&to=2025-06-01T18:00:00&sort=capacity,desc&page=0&size=20
Authorization: Bearer <JWT>
```
...
| `keyword` | string | ❌ | `name` / `description` への部分一致でフィルタ（大文字小文字を区別しない） |
...
>
> `keyword` は `category` / `from` / `to` / `sort` と AND 条件で組み合わされる。空文字または前後の空白のみを除去した結果が空文字になる場合は、未指定時と同じ扱い（フィルタ解除）となる。
>
