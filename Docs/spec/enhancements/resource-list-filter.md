...
- 競合する課題：[リソース一覧のソート順選択](./resource-list-sort.md) — 両課題とも `GET /api/resources` と同一の `ResourceFilterForm.tsx` を変更するため、並行着手は非推奨。
- 推奨着手順序：本課題（キーワード検索）を**先に着手**するとよい。[リソース一覧のソート順選択](./resource-list-sort.md) の受入条件がキーワード検索との組み合わせを前提としているため。後続として [OpenAPI クライアント自動生成](./openapi-client-gen.md)・[既存機能の E2E テスト追加](./e2e-test-coverage.md) がある。
