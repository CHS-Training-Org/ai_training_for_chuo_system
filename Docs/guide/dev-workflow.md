...
| plan-first のセルフ承認 | Claude Code **plan mode** でエンジンが Workflow Planning を提示 → 学習者自身が `ExitPlanMode` で計画に納得したことを示してから実装に進む |
...
### 3. Claude Code の plan mode で AI-DLC エンジンが INCEPTION フェーズを実行する
