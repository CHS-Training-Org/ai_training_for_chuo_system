...
|---|---|
| Inception（WHAT/WHY） | `/aidlc` 起動後、plan mode 内でエンジンが Workspace Detection → Requirements Analysis → Workflow Planning を実行 |
...
| plan-first のセルフ承認 | `/aidlc` 起動時、Claude Code **plan mode** でエンジンが Workflow Planning を提示 → 学習者自身が `ExitPlanMode` で計画に納得したことを示してから実装に進む |
...
計画段階（plan mode）・実装完了段階（PR）のそれぞれで、学習者自身がセルフチェックしてから次に進みます。メンターは必須の承認者ではなく、Issue・PR への任意のコメントで支援します。
...
取り組む課題を Issue から選びます。Issue にはビジネス要求シート（背景、要件、受入条件、影響範囲、AI 活用ポイント）への参照が含まれます。  
受入条件はシート側が真実の源です。
...
!!! tip "コードベースを読み解くタイミング"
    実装に入る前に対象機能のコードを読み解いておくと、次の plan mode での計画が立てやすくなります。読み方の目安は [curriculum.md §コードベース理解ガイド](./curriculum.md#codebase-understanding) を参照してください。

### 3. `/aidlc` を起動して AI-DLC エンジンに INCEPTION フェーズを実行させる

shift+tab でプランモードに切り替え、ビジネス要求シートの内容を伝えたうえで、`/aidlc` を起動する（または「AI-DLC で進めて」と明示的に伝える）と、AI-DLC エンジン（`.claude/skills/aidlc/SKILL.md`）が発動します。AI-DLC の指定がない小修正・質問では発動しません。  
...
計画の内容を自分で確認し、納得したら `ExitPlanMode` で承認して実装に進みます。メンターの承認は不要です。計画に問題があればこの段階で修正します。  
plan mode の詳しい使い方は [ai-tools-guide.md](./ai-tools-guide.md) を参照してください。
