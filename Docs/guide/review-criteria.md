...
観点3だけは3値ではなく**状態**（`未回答` / `全問正解` / `誤答N問・解説済み`）で表示されます。**観点1・観点2が `OK`、CI green、かつ観点3が確定しているとき**に総合判定が「完了」になり、これがタスク完了の条件です。観点3の誤答は完了を妨げません（[ADR-026](../decision/ADR-026-comprehension-check-quiz-format.md)）。誤答はコードの欠陥ではなく理解度であり、修正して解消できる種類のものではないためです。ただし誤答は PR コメントに記録として残ります。
