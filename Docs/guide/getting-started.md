...
ロール別ログイン（開発用ログイン）は、バックエンドが Cognito トークンの `sub` で `users` テーブルを検索する仕組みのため、初期データを投入する前に試すとサインイン画面へのリダイレクトを繰り返します（原因は [troubleshooting.md](./troubleshooting.md#startup) 参照）。先に初期データを投入してから、起動確認に進みます。
...
**手順 1：postgres コンテナ名を確認する**

```bash
docker ps --format "table {{.Names}}\t{{.Image}}" | grep postgres
```

表示例：`ai_training_for_chuo_system_devcontainer-postgres-1`
...
```bash
docker exec -i ai_training_for_chuo_system_devcontainer-postgres-1 \
...
続いて <http://localhost:3000> を開き、サインイン画面のロール別ログインボタンでサインインします。ダッシュボードにサンプルデータが表示されれば、環境構築は完了です。

| 確認項目 | 期待値 |
|---------|--------|
| リソース（`/resources`） | 3 件（ROOM・EQUIPMENT・VEHICLE 各 1 件） |
| 予約 | 2 件（APPROVED 1 件・PENDING 1 件） |
| 承認待ち（`/approvals`） | 1 件（第1会議室の申請） |

!!! warning "サインインが `/auth/signin` に戻ってしまう場合"
    上記の初期データ投入を先に済ませたか確認してください。原因の切り分けは [troubleshooting.md](./troubleshooting.md#startup) を参照してください。
