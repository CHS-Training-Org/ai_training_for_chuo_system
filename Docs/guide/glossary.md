...
詳細は [dev-workflow.md](./dev-workflow.md) と `.claude/rules/aidlc-core.md` を参照してください。

| 用語 | 説明 |
|------|------|
| **AI-DLC** | AWS Labs の AI Development Life Cycle（[`awslabs/aidlc-workflows`](https://github.com/awslabs/aidlc-workflows)）。BookFlow が標準開発フローとして採用しているエンジン |
| **INCEPTION フェーズ** | AI-DLC の第 1 フェーズ（WHAT/WHY）。plan mode で Workspace Detection → Requirements Analysis → Workflow Planning を実行し、実行計画を提示する |
...
| **units of work** | CONSTRUCTION で並行実行可能な作業単位。BookFlow では縦切り課題 Issue（`feature/<issue番号>-<short-desc>` 単位）に対応する |
| **縦切り実装** | フロントエンド・バックエンドなど複数レイヤーにまたがる変更を機能単位でまとめて実装する方針 |
| **Spec-first** | 実装より先に `Docs/spec/` を更新し、それを真実の源とする原則 |
| **第 1 ゲート** | plan mode でエンジンが提示する Workflow Planning に対するメンターの承認ゲート（計画段階） |
| **第 2 ゲート** | PR に対するメンターレビュー・Approve のゲート（実装完了段階） |
