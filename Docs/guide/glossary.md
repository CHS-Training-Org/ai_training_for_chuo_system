...
| **必須課題** | 環境構築など順序依存のあるステップ課題（STEP-01〜03）。GitHub Issue は起票せず自己チェックで進める。新人は先にこれをクリアする |
...
| **INCEPTION フェーズ** | AI-DLC の第 1 フェーズ（WHAT/WHY）。`/aidlc` 起動後、plan mode 内で Workspace Detection → Requirements Analysis → Workflow Planning を実行し、実行計画を提示する |
