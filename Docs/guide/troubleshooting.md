...
- **解決策**: リポジトリのルートを開き直します。WSL2 ターミナルでルートへ移動して `code .` を実行するか、VS Code の **File → Open Folder** でルートを選び直してください。
    ...
    開き直してもプロンプトが出ない場合は、コマンドパレット（++ctrl+shift+p++）から「**Dev Containers: Reopen in Container**」を手動で実行します。
...
- **解決策**: リモートを共有リポジトリに戻し、作業ブランチを push し直します。コミットはそのまま使えます。
    ...
    2. 作業ブランチに切り替え、`git push -u origin <ブランチ名>` で共有リポジトリへ push する。`-u` を付けると以降の追跡先も共有リポジトリになる。
    3. 共有リポジトリ側で PR を作成し直す。fork 側に作ってしまった PR は close する。
    4. fork リポジトリは不要なので、共有リポジトリ側にコミットが揃っていることを確認してから削除してよい（自分のアカウントの当該リポジトリ → Settings → Danger Zone → Delete this repository）。削除は取り消せないため、確認してから実行する。ローカルの参照だけ外す場合は `git remote remove fork`。

!!! warning "fork のまま進めない"
    fork 側では、AI レビュー（`@claude pr-review`）が依存する secrets、ラベル、Issue、ブランチ保護がいずれも用意されていません。PR も共有リポジトリに現れないため、運営者からレビューも進捗把握もできません。作業は必ず共有リポジトリ内のブランチで行ってください。
...
`@claude pr-review` で起動する AI レビューについての症状です。仕組みと判定基準は [review-criteria.md §AI レビューとの対応](./review-criteria.md#ai-review)、運用の詳細は [operations-guide.md §AI レビュー](./operations-guide.md#ai-review) を参照してください。

### コメントしても反応がない

- **症状**: PR に `@claude pr-review` と書いたが、しばらく何も投稿されない。
- **原因**: レビューには時間がかかります。実測で **10〜15 分程度**です。差分が大きいほど長くなります。
- **解決策**: まず待ちます。**同じコメントを連投しないでください。** 同一 PR に対する AI レビューは直列化されるため、連投しても順番待ちが増えるだけです。15 分以上経っても投稿がない場合は、リポジトリの Actions タブで `Claude Code` ワークフローの実行結果を確認してください。実行自体が始まっていない場合は、コメントの書き出しが `@claude pr-review` になっているかを見直します（`@claude review` などでは AI レビューは起動しません）。

### 前提条件の CI green が「判定不能」になる

- **症状**: サマリの「前提条件 CI green」が `判定不能` と表示され、総合判定が「未完了」のままになる。
- **原因**: push 直後に `@claude pr-review` を実行すると、CI がまだ実行中（pending）でレビュー側が結果を読めません。
- **解決策**: CI の完了を待ってから `@claude pr-review` をやり直してください。

### 観点3の問題に回答したのに読み取ってもらえない

- **症状**: 4択に回答したのに、次のレビューでも観点3が `未回答` のままになる。
- **原因**: 回答として扱われるのは **PR 作成者本人**が投稿した**会話コメント**だけです。差分の行に付けるレビューコメントやレビュー本文は読み取れません。
- **解決策**: PR 画面下部の会話コメント欄に、「問番号＋選んだ選択肢＋選んだ理由を一文」の形式（例：`1-B。理由は…`）で投稿し直してください。全問に回答が揃わないと判定されず、解説も返りません。

### 観点3をもう一度受け直したい

- **症状**: 観点3が `確定` になった後、実装を大きく書き直したので改めて理解度チェックを受けたい。
- **原因**: 観点3は一度確定すると再判定・再出題しません。判定が実行ごとに揺れるのを避けるためです。確定後にコミットが追加された場合は、サマリにその旨が添えられます。
- **解決策**: 観点3のコメント（先頭が `**AI レビュー / 観点3 理解度チェック ...**` のもの）を削除してから `@claude pr-review` をやり直してください。状態はこのコメントから復元されるため、削除すると未出題の扱いに戻り、現在の差分に対して出題し直されます。
