...
- 計画承認の確認自体は、エンジンが提示した計画に対して学習者がチャットで直接返答する形で行う。`AskUserQuestion` で改めて「この計画でよいですか？」と問い直さない（[`docs-next/docs/develop/dev-workflow.md` §3](../../docs-next/docs/develop/dev-workflow.md)）。承認するのは学習者自身であり、メンターの承認は不要。
