...
- **plan mode 第1ゲート**: Workflow Planning ステージの完了時に `ExitPlanMode` を呼び、学習者（メンター）の承認を得る。
...
4. **Wait for Explicit Approval** — emphasize user control to override recommendations. BookFlow では `ExitPlanMode` を使って承認を得る（plan mode 第1ゲート）
5. **MANDATORY**: Log response in `Docs/spec/aidlc-audit.md`
