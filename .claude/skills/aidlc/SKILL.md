...
2. 存在しない場合（新規ワークフロー開始時のみ）、現在の git ブランチを確認する。
   - **`feature/<GitHubユーザー名>/<issue番号>-<short-desc>` 規約に既に合致**：`Docs/guide/dev-workflow.md` 手順2で作成済みのケースなので、何もせず次節に進む。
   - **`main` または `master` 上にいる**：Issue 番号と短い説明をユーザーに確認する。GitHub ユーザー名は `gh api user -q .login` で取得し（取得できない場合のみユーザーに確認する）、`git checkout -b feature/<user>/<issue>-<desc>` を実行してから次節に進む。
   - **`main`/`master` でも規約準拠でもない**（別の作業ブランチに乗ったまま起動した場合）：現在のブランチ名を明示したうえで「このまま続ける」か「新しく feature ブランチを作成する」かを `AskUserQuestion` で確認してから次節に進む。
