...
（旧 `.claude/rules/aidlc-core.md` にあった「plan mode 経由で発動／OVERRIDES を plan mode 第1ゲートと統合」という弱め読み替えは**このスキルでは適用しない**。起動＝エンジン開始）

---

## BookFlow 統合ノート

- **plan mode でのセルフ承認**: Workflow Planning ステージの完了時に `ExitPlanMode` を呼び、学習者自身が計画を確認・承認する（メンターの承認は不要）。
...
4. **Wait for Explicit Approval** — emphasize user control to override recommendations. BookFlow では `ExitPlanMode` を使って学習者自身が承認する（メンターの承認は不要）
5. **MANDATORY**: Log response in `Docs/spec/aidlc-audit.md`
