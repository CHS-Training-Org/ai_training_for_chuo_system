...
| `バグ報告` | `[バグ]` |
| `質問・相談` | `[質問]` |
| `課題` | プレフィックス不要（STEP / Enhance 種別で設定済み） |
...
- メンターが運用する追加ラベル（`順序依存あり` / `対象：*` / `進行中`）は**このスキルでは付けない**。起票後にメンターが [§ラベルマッピング規則](../../../Docs/guide/issue-registration.md#mapping) に従って手動付与する旨を一言添える。
...
- **ラベル実体が無いときの挙動は経路で異なる**：
  ...
  - **gh の `--label` はサイレントに無視されず、ハードエラーになる**（`could not add label: 'X' not found` 等）。gh はラベル名を API で解決してから起票するため、未登録ラベルを指定すると **issue 自体が作られず失敗する**。起票前に `gh label list` でラベル実体を確認すること（§4.3 の表は最後に確認した実在ラベル一覧だが、変更されている可能性があるため都度確認が望ましい）。
...
- **メンター運用ラベルを勝手に付けない**：`順序依存あり` / `対象：*` / `進行中` はメンターが付与する。`難易度：*` はスキルが自動付与するため、メンター付与は不要（§4.1〜4.3 の難易度選択に従う）。
- **タイトルプレフィックスは性質ラベルから自動決定する**（§4.3 の対応表）。プレフィックス未設定のまま起票しない。起票前の確認ゲート（§5）でプレビューに含めること。
