...
4. **Render a sample** using the sample-diagram skeleton in `references/style-extraction.md`, parameterized by the candidate preset. Rendering は VSCode の `hediet.vscode-drawio` 拡張で確認する（CLIエクスポートおよび repair_png.py は使用しない）。詳細手順は `references/style-extraction.md` の「Rendering the sample」を参照。
5. **Show the user:**
