...
2. VSCode で `/tmp/drawio-preset-<name>.drawio` を開き、`hediet.vscode-drawio` 拡張でレンダリングを確認する。PNG が必要な場合はコマンドパレット（Ctrl+Shift+P）→「Draw.io: Export...」で `./preset-<name>-sample.png` に書き出す。CLIエクスポートおよび repair_png.py は使用しない。
3. 拡張が使えない場合: `python3 <this-skill-dir>/scripts/encode_drawio_url.py --edit /tmp/drawio-preset-<name>.drawio` でブラウザ URL を生成し、ブラウザ上で確認する。
4. Show the user: preset summary table + ファイルパスまたはブラウザURL + provenance/confidence line.
...
### If sample render fails (extension unavailable)

Still show the summary table and the provenance line. Note: *"Could not render sample (VSCode extension unavailable). Save anyway on your OK."* Do not block.
